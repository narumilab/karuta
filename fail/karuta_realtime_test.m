function [recog_fuda, kimariji_second] = karuta_realtime_test(input_wav_path, deviceReader) % ⭐第2引数を追加
    ...
    % ⭐ 2. バッファの「掃除（空読み）」を実行
    % これにより、pause(0.5)中に溜まった古いデータをすべて捨てます
    while ~isDone(deviceReader)
        [~, numOverrun] = deviceReader();
        if numOverrun == 0
            break; % 最新のデータに追いついたらループを抜ける
        end
    end
    sound(y_play, Fs_play); % 再生開始
