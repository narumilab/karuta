...
for test=1:10
    ...
    list = cell(3,2);
    list{1,1} = 1;
    list{1,2} = sprintf('mfcc_silence_cut/ooe%d.mfc',test);
    list{2,1} = 2;
    list{2,2} = sprintf('mfcc_silence_cut/ooke%d.mfc',test);
    list{3,1} = 3;
    list{3,2} = sprintf('mfcc_silence_cut/ooko%d.mfc',test);
    save('testing_list.mat','list');
    list = cell(27,2);
    k = 1;
    for i=1:10
        if i==test
        ...
        list{k,1} = 1;
        list{k,2} = sprintf('mfcc_silence_cut/ooe%d.mfc',i);
        k = k+1;
        list{k,1} = 2;
        list{k,2} = sprintf('mfcc_silence_cut/ooke%d.mfc',i);
        k = k+1;
        list{k,1} = 3;
        list{k,2} = sprintf('mfcc_silence_cut/ooko%d.mfc',i);
        k = k+1;
    end
    save('training_list.mat','list');

    AIC = zeros(3,MAX_EMIT_STATE_NO);
    for EMIT_STATE_NO=1:MAX_EMIT_STATE_NO
        N=EMIT_STATE_NO+2;
        model_filename_prefix='models_silence_cut/EM_models';
        ...
        model_filename=['models_silence_cut/EM_models_S', int2str(EMIT_STATE_NO), '_iter' int2str(ITERATION_END) '.mat'];
        ll = zeros(3,10);
        for i=1:10
            if i==test
                continue
            ...
            for fuda=1:3
                if fuda==1
                    filename = sprintf('mfcc_silence_cut/ooe%d.mfc',i);
                end
                if fuda==2
                    filename = sprintf('mfcc_silence_cut/ooke%d.mfc',i);
                end
                if fuda==3
                    filename = sprintf('mfcc_silence_cut/ooko%d.mfc',i);
                end
                ...
                ll_tmp = recognition_Viterbi_hmm_skips_1gau_no_terminal(c,fuda,model_filename);
                ll(fuda,i) = ll_tmp(fuda,end);
            end
        end
    
        AIC(1,EMIT_STATE_NO) = -2*(sum(ll(1,:)))+2*EMIT_STATE_NO+2*(39+39)*EMIT_STATE_NO;
        AIC(2,EMIT_STATE_NO) = -2*(sum(ll(2,:)))+2*EMIT_STATE_NO+2*(39+39)*EMIT_STATE_NO;
        AIC(3,EMIT_STATE_NO) = -2*(sum(ll(3,:)))+2*EMIT_STATE_NO+2*(39+39)*EMIT_STATE_NO;

    end
    [min1,num_state1] = min(AIC(1,:));
    [min2,num_state2] = min(AIC(2,:));
    [min3,num_state3] = min(AIC(3,:));
    
    model_filename1=sprintf('models_silence_cut/EM_models_S%d_iter%d.mat',num_state1,ITERATION_END);
    model_filename2=sprintf('models_silence_cut/EM_models_S%d_iter%d.mat',num_state2,ITERATION_END);
    model_filename3=sprintf('models_silence_cut/EM_models_S%d_iter%d.mat',num_state3,ITERATION_END);

    for fuda=1:3
        if fuda==1
            filename = sprintf('mfcc_silence_cut/ooe%d.mfc',test);
        end
        if fuda==2
            filename = sprintf('mfcc_silence_cut/ooke%d.mfc',test);
        end
        if fuda==3
            filename = sprintf('mfcc_silence_cut/ooko%d.mfc',test);
        end
        fid=fopen(filename,'r');
        fseek(fid, 12, 'bof'); % skip the 12-byte HTK header
        c=fread(fid,'float','b');        
        ...
        ll_tmp = recognition_Viterbi_hmm_skips_1gau_no_terminal(c,1,model_filename1);
        ll(1,1:fr_no) = ll_tmp(1,:);
        ll_tmp = recognition_Viterbi_hmm_skips_1gau_no_terminal(c,1,model_filename2);
        ll(2,1:fr_no) = ll_tmp(2,:);
        ll_tmp = recognition_Viterbi_hmm_skips_1gau_no_terminal(c,1,model_filename3);
        ...
        k = 1;
        for t=1:fr_no
            if posterior_series(fuda,fuda,t) > thresholds(k)
%            if min(posterior_series(fuda,fuda,t:fr_no)) > thresholds(k)
    ...
    output_filename=sprintf('final%d_silence_cut.mat',test);
    save(output_filename,'kimariji','thresholds','posterior_series','num_state*','AIC');
end
